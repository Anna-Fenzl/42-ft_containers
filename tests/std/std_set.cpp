...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/09 16:56:04 by afenzl            #+#    #+#             */
/*   Updated: 2023/02/09 16:56:41 by afenzl           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void test_set()
{
	...
	RESET
	{
		...
		std::set<std::string>	range(empty.begin(), empty.end());
		// range.print(); NEWLINE BORDER

		BOLD std::cout << "--> COPY SET:"; RESET 
		std::set<std::string>	copy(empty);
	...
	
	{
		...
		for (std::set<char>::iterator it = iter.begin(); it != iter.end(); ++it )
		{
			std::cout << *it << ", ";
		} NEWLINE BORDER

		BOLD std::cout << "RBEGIN() TO REND()"; RESET
		for (std::set<char>::reverse_iterator it = iter.rbegin(); it != iter.rend(); ++it )
		{
			std::cout << *it << ", ";
		} NEWLINE BORDER
	}
...

	{
		...
		const std::set<int> money(coins);
		// money.print(); NEWLINE
	
		const std::set<int>::const_iterator it = money.find(100);
